...
public class Biblioteca2025HugoIglesias {
    ...
    public Biblioteca2025HugoIglesias() {
    ...
    public static void main(String[] args) {
        Biblioteca2025HugoIglesias b= new Biblioteca2025HugoIglesias();
        b.cargaDatos();
        b.fueraPlazo();
        b.menu();
    }
...
//</editor-fold>
    ...
    for (int i = 0; i < contadoresLibrosPorAutor.length; i++) {
     ...
     //Cree otra clase llamada Reserva.java para incluir las siguientes opciones
...
private void nuevoPrestamoConLimite() {
    int limitePrestamos = 2;
    ...
    if (posUsuario == -1) {
        ...
        if (prestamosActivos >= limitePrestamos) {
            System.out.println("El usuario ya tiene el número máximo de préstamos (" + limitePrestamos + ").");
            return;
        }
        ...
        if (posLibro == -1) {
            ...
            if (buscaPrestamo(dni, isbn) == -1) {
                ...
                System.out.println("Ese usuario ya tiene ese mismo libro en préstamo.");
            }
        } else {
            System.out.println("No quedan unidades disponibles del libro.");
        }
...
/*private void mostrarMultasUsuario() {
    ...
    if (posUsuario == -1) {
        ...
        for (Prestamo p : prestamosHist) {
            if (p.getUsuarioPrest().getDni().equals(dni)) {
                if (p.getFechaDevReal().isAfter(p.getFechaDev())) {
                    long diasRetraso = p.getFechaDevReal().toEpochDay() - p.getFechaDev().toEpochDay();
                    totalMultas += diasRetraso * 0.5; // Multa de 0.5 por día
                }
            }
        }
        System.out.println("El usuario debe: $" + totalMultas + " en multas.");
    }
}
*/
